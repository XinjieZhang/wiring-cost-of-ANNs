...
from model.structure_evolution import weight_sampler_strict_number, assert_connection_number, rewiring, rewiring_improved
...
def train(model_dir,
    ...
    with open(os.path.join('../datasets', 'random_network', str(hp['graph_file']))) as f:
        reader = csv.reader(f)
        header = next(reader)
        for row in reader:
    ...
    normalized = True
    if normalized:
        sigma_D = np.sqrt(np.sum(distance_matrix * distance_matrix) / n_rnn / (n_rnn - 1))
        hp['l1_weight'] = hp['l1_weight'] / sigma_D
        tools.save_hp(hp, model_dir)
    ...
    for epoch in range(maxepoches):
        ...
        with tf.Session() as sess:
            ...
            w_rec, w_rec_mask, nb_reconnect = rewiring_improved(theta=w_rec0 * w_rec1,
                                                                weights=w_rec1,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modeldir', type=str, default='experiments/DMTask/rewiring_DR/DeepR_with_l1_cost')
    args = parser.parse_args()

    os.environ["CUDA_VISIBLE_DEVICES"] = "0"
    hp = {'activation': 'softplus',
          'w_rec_init': 'randortho',
          'graph_file': 'node_coordinate_256.csv',
          'n_rnn': 256,
          'learning_rate': 0.01,
          'epsilon_rec': 1,
          'num_edges': 10000,
          'l1_weight': 1e-4,  # L1 + wiring cost
          'l2_weight': 0,  # L2 + wiring cost
          }
    train(args.modeldir,
          ruleset='dm',
          seed=2,
