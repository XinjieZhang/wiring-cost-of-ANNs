...
def largest_connected_component_of_Celegans():
    ...
    remove = [node for node in G.nodes() if node not in largest_cc]
...
if __name__ == '__main__':
    ...
    G = largest_connected_component_of_Celegans()
    ...
    with open(os.path.join('../datasets', 'C. elegans', 'spatialpositions', 'distances.csv')) as f:
        ...
        header = next(reader)
        for row in reader:
            meta[row[0]] = (float(row[1]), float(row[2]), float(row[3]))
    ...
    for i in range(len(nodes)):
        for j in range(i+1, len(nodes)):
            distance.append(Euclidean_distance(meta[nodes[i]], meta[nodes[j]]))
    D_max = np.max(distance)

    x1, dp1 = cumulative_distribution(G=G, meta=meta, d_max=D_max)
    ...
    DATAPATH = os.path.join(os.getcwd(), '../results', 'Person', 'C_elegans_connectom')
    model_dir = os.path.join(DATAPATH, 'test')

    frontal_meta = dict()  # [node_id, posx, posy, posz]
    node = []
    with open(os.path.join(model_dir, 'C-elegans-connectom.csv')) as f:
        reader = csv.reader(f)
        header = next(reader)
        for row in reader:
            node.append(row[1])
    ...
    csv_file = 'edge_list_weighted_150.csv'
    G0 = nx.DiGraph()
    ...
    plt.plot(x1, dp1, '-', label='$C. elegans$ connectome')

    ax = sns.lineplot(x=x, y=y, label='Trained GRU')
    ax1 = sns.lineplot(x=x_random, y=y_random, label="Random position", color='grey')
    plt.xlim(0, 1.05)
    plt.ylim(1e-3, 1)
    plt.yscale('log')
